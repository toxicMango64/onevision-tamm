...
def get_train_transforms():
    return v2.Compose([
        v2.Resize(256),  # First resize to 256
        v2.RandomResizedCrop(224),  # Then random crop to 224
        v2.RandomHorizontalFlip(p=0.5),
        v2.ToImage(),
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
#     ref_emb = ref_embeddings.cpu().numpy() if torch.is_tensor(ref_embeddings) else ref_embeddings
#     query_emb = query_embeddings.cpu().numpy() if torch.is_tensor(query_embeddings) else query_embeddings
#     ref_labs = ref_labels.cpu().numpy() if torch.is_tensor(ref_labels) else ref_labels

#     # Determine precision
#     dtype = np.float16 if use_float16 and faiss.Float16Supported() else np.float32
#     ref_emb = ref_emb.astype(dtype)
#     query_emb = query_emb.astype(dtype)
...
#     config.device = 0
#     config.useFloat16 = use_float16 and faiss.Float16Supported()
...
#         gpu_index.add(ref_emb)
        
#         faiss.normalize_L2(query_emb)
#         distances, indices = gpu_index.search(query_emb, k)
        
#         return torch.from_numpy(ref_labs[indices])
...
def prepare_dataloaders(path, train_csv, valid_csv, batch_size, use_arc_margin=False):
    ...
    if use_arc_margin:
        ...
        train_loader = DataLoader(train_data, batch_size=batch_size, sampler=sampler, 
                             num_workers=min(4, os.cpu_count()-1), pin_memory=True, persistent_workers=True, prefetch_factor=4)
...
def main():
    ...
    #path = os.path.join(os.getcwd(), "Task-1-Pawsitive", "data")
