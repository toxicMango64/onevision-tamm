    ...
    return v2.Compose(
        [
            v2.RandomResizedCrop(
                224, scale=(0.6, 1.0), ratio=(0.9, 1.1), antialias=True
            ),
            ...
            v2.RandomAffine(
                degrees=25, translate=(0.05, 0.05), scale=(0.85, 1.15), shear=10
            ),
...
def get_knn_labels(
    ref_embeddings, query_embeddings, ref_labels, k=5, use_float16=False
):
    ref_embeddings = (
        ref_embeddings.cpu().numpy()
        if torch.is_tensor(ref_embeddings)
        else ref_embeddings
    )
    query_embeddings = (
...
def prepare_dataloaders(path, train_csv, valid_csv, batch_size, use_arc_margin=False):
    ...
    if use_arc_margin:
        ...
        sampler = samplers.MPerClassSampler(
            train_data.dataframe["label"].values,
            m=8,
            batch_size=batch_size,
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % validate_every == 0 or epoch == epochs - 1:
            ...
            with torch.inference_mode():
                ...
                print(
                    f"Epoch {epoch + 1}: Loss={avg_loss:.4f}, Retrieval@{k}={retrieval_score * 100:.2f}%"
                )
...
def main():
    ...
    train_csv, valid_csv = redistribute_classes(
        train_csv, valid_csv, test_size=0.2, random_state=42
    )
    ...
    train_loader, val_loader, num_classes = prepare_dataloaders(
        path, train_csv, valid_csv, config["batch_size"], config["use_arc_margin"]
    )

    print(
        "Unique labels:",
        torch.unique(torch.tensor(train_loader.dataset.dataframe["label"])),
    )

    model = ResNetFE(
        config["embedding_size"], weights=ResNet50_Weights.IMAGENET1K_V2
    ).to(device)
